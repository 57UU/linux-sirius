...
static int fts_pinctrl_init(struct fts_ts_info *info)
{
	...
	info->ts_pinctrl = pinctrl_get(info->dev);
...
static int fts_probe(struct spi_device *client)
{
	...
	/*
	 * Mainline sirius port: no display notifier exists to deliver
	 * the UNBLANK event, so run the resume path once here to turn
	 * sensing on. Mirrors what the MSM DRM blank callback would do.
	 */
	queue_work(info->event_wq, &info->resume_work);
	return OK;
