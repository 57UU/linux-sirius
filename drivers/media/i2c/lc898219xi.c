...
static int lc898219xi_set_dac(struct lc898219xi *lc898219xi, u16 val)
{
	...
	ret = cci_write(lc898219xi->regmap, LC898219XI_DAC_ADDR, val, NULL);
	if (ret)
...
static int lc898219xi_power_on(struct lc898219xi *lc898219xi)
{
	...
	dev_warn(&client->dev, "writing enable bit\n");
	client->addr = 0x73;
	EEPROM_3Fh = i2c_smbus_read_byte_data(client, 0x3F);
	client->addr = addr_bak;
	uint32_t regdata = i2c_smbus_read_byte_data(client, 0xF0);
	if (regdata == 0xA5) {
		dev_info(&client->dev, "check communication success\n");
	} else {
		dev_err(&client->dev, "check communication error regdata: %x \n", regdata);
		// return -1;
	}
...
static int lc898219xi_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	dev_warn(&client->dev, "catikato %d\n", ctrl->id);
